...
import com.icebergv01.service.UosUifDetailsService;
...
public class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	@Autowired
	private UosUifDetailsService uosUifDetailsService;
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		auth.userDetailsService(uosUifDetailsService).passwordEncoder(passwordEncoder());
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/").permitAll().antMatchers("/login").permitAll()
				...
				.defaultSuccessUrl("/admin/index").usernameParameter("user_name").passwordParameter("password").and()
